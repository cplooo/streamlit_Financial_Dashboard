...
# KBar_df['RSI_long']=RSI(KBar_dic,timeperiod=LongRSIPeriod)
# KBar_df['RSI_short']=RSI(KBar_dic,timeperiod=ShortRSIPeriod)
...
## 尋找最後 NAN值的位置
#last_nan_index_RSI = KBar_RSI_df['RSI_long'][::-1].index[KBar_RSI_df['RSI_long'][::-1].apply(pd.isna)][0]
...
with st.expander("長短 RSI"):
    ...
    # # include candlestick with rangeselector
    # fig1.add_trace(go.Candlestick(x=KBar_RSI_df['Time'],
    #                 open=KBar_df['Open'], high=KBar_RSI_df['High'],
    #                 low=KBar_RSI_df['Low'], close=KBar_RSI_df['Close'], name='K線'),
    #                secondary_y=True)   ## secondary_y=True 表示此圖形的y軸scale是在右邊而不是在左邊
